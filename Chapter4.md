...
**always_ff**は論理合成時にフリップフロップとして扱われることを指示しており```@()```の中にイベントの対象となるオブジェクト名を記述して明示します。
...
これはSystemVerilogも含めたHDLは単にハードウェア記述言語であり、リスト（センシティビティリストと呼びます）内で指定されたイベントを基に例えばシミュレーション上でそのリストに従ってイベントを発生させて代入させることに起因しています。
...
always_combと**for**文を組み合わせることで簡易に記述できるようになります。例えば単ビット演算の
...
always_comb内ではassignを使用して割り当てるので三項演算子も使用できます。
...
のようにfor文でgenvarで定義した整数iを使用して代入を行い記述できます。
